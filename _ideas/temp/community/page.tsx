...
} satisfies Record<"en"|"tet",PageContent>

export default function ImpactStoriesPage(){
  const [search,setSearch]=useState("")
  const {language}=useLanguage()

  const safeLanguage=language==="en"?"en":"tet"
  const t=content[safeLanguage]
  ...
  return(
    <main className="min-h-screen bg-white p-8">
      <h1 className="mb-6 text-4xl font-bold text-green-700">{t.heading}</h1>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        {filteredStories.map((story)=>(
          <div key={story.id} className="rounded-lg border p-4 shadow transition hover:shadow-md">
            <h2 className="mb-2 text-xl font-semibold text-red-700">{story.title}</h2>
