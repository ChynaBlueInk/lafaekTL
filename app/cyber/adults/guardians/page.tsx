...
export default function GuardiansInteractivePage(){
  ...
  const t={
    ...
    tet:{
      ...
      subtitle:
        "Atividade badak no prátiku atu ajuda ita ko'alia ho labarik sira kona-ba liafuan segredu, limite sira, konfigurasaun privasidade, no ema seluk online.",
      badge:"Interativu",
      back:"← Fila ba Inan-Aman & Kuidadór",
      sectionTalk:"Ajuda Konversa",
      sectionPasswords:"Seguransa Liafuan Segredu (Demonstrasaun Simples)",
      sectionBoundaries:"Kria regulamentu Família nian",
      sectionPrivacy:"Lista Cheka Konfigurasaun Privasidade",
      sectionScenarios:"Atensaun ba senáriu sira nee",
      ...
      topics:[
        {id:"passwords",label:"Sena no Fraze segredu"},
        {id:"privacy",label:"Privasidade no fahe informasaun sen limite"},
        {id:"boundaries",label:"Regra no limitasaun ba utilizasaun telemovel/komputadór"},
        {id:"strangers",label:"ema seluk, foto, enkontru"},
      ],

      demoIntro:
        `La presiza perfeitu. Objetivu mak halo ema bosokteen sira susar liu. Fraze segredu naruk di'ak liu liafuan segredu badak ne'ebé "moris".`,
      demoLabel1:"Ezemplu liafuan segredu",
      demoLabel2:"Ezemplu fraze segredu",
      demoHint1:"Koko: P@ssw0rd123",
      demoHint2:"Koko: mango river turtle 9!",
      demoResultTitle:"Saida mak demonstrasaun ida-ne'e hatudu",
      demoResult:
        "Fraze naruk fasil atu lembra no susar atu adivinha. Objetivu: fraze segredu úniku ba konta ida-idak + MFA iha konta importante sira (email, sosiál, banku).",

      boundaryIntro:
        "Hili regra balun ne'ebé diak ba ita nia uma. Halo realistiku. se ita la aplika, regra neebe ita labele aplika laiha valor.",
      boundaryCopy:"Kopia testu akordu",
      boundaryCopied:"Kopiadu!",
      boundaryTitle:"Akordo konaba utilizasaun teknologia iha familia nia laran.",

      privacyIntro:
        "Konfigurasaun sira ne'e normalmente mak causa problema. Hamate/halakan (on/off) atu haree ou nota risku.",
      privacyOn:"RISKU",
      privacyOff:"SEGURU LIU",

      scenariosIntro:
        "Mensajen komun sira ne'e mak labarik sira dalabarak simu. Hili resposta ida atu haree saida mak di'ak liu.",
      restart:"Hahu fali",
  ...
  const scripts=useMemo(()=>getScripts({age,language}),[age,language]);
  ...
  const ruleOptions=useMemo(()=>[
    {id:"noBedrooms",label:language==="tet"?"Bandu hodi utilija sasan elektroniku sira hanesan telemovel iha tempo kalan":"No devices in bedrooms overnight"},
    ...
    {id:"pauseThinkTell",label:language==="tet"?"Pauza–Hanoin–Hatete bainhira sente urjente/segredu":"Pause–Think–Tell if a message feels urgent/secret"},
  ...
  const privacyItems=useMemo(()=>[
    {
      ...
      desc:language==="tet"?"Apenas kolega/aprovadu mak bele haree publikasaun sira.":"Only approved friends can see posts.",
    ...
    },
    {
      ...
      title:language==="tet"?"Limita mensagen direta":"Limit direct messages",
      desc:language==="tet"?"Permite de'it kolega ka ema aprovadu atu haruka mensajen.":"Allow DMs only from friends/approved people.",
      risk:language==="tet"?"Mensagen direta livre = fasil atu grooming/scam.":"Open DMs increase grooming/scam risk.",
    },
    {
    ...
    },
    {
      ...
      title:language==="tet"?"Hamate ita nia lokasaun atual":"Disable precise location",
      desc:language==="tet"?"Desliga lokasaun atual iha app sosiál/jogu.":"Turn off precise location in social/gaming apps.",
      risk:language==="tet"?"Lokasaun atual = risku hasoru/monitoriza.":"Precise location increases stalking/meet-up risk.",
    },
    {
      key:"reviewTags" as const,
      title:language==="tet"?"Halo revizaun ba marka naran iha imagen sira":"Review tags",
      desc:language==="tet"?"Presiza aprovasaun molok ema marka naran ita iha foto.":"Require approval before being tagged.",
      risk:language==="tet"?"Marka naran livre = foto/info sai publiku la hatene.":"Unapproved tags can expose info without consent.",
    },
    {
              ...
              <div key={item.key} className="rounded-xl border border-[#E5E7EB] p-4">
                ...
                <div className="mt-3 flex items-start gap-2 rounded-lg bg-[#F5F5F5] p-3">
                  ...
                  <div className="text-sm text-[#4F4F4F]">
                    {privacy[item.key]?item.risk:(language==="tet"?"Konfigurasaun ida-ne'e hamenus risku.":"This setting reduces risk and makes problems less likely.")}
                  </div>
                </div>
              </div>
            ))}
...
function getScripts({age,language}:{age:AgeBand;language:"en"|"tet"}){
  if(language==="tet"){
    const tet={
      passwords:{
        "8-12":"Tenta ida ne'e:\n\"Ita uza liafuan segredu hodi taka ita nia konta. La bele fahe ba ema seluk, maski ba kolega. Se haluha, hatete mai hau — la bele adian.\"\n\nKontinua ho: \"Mai ita kria ida hamutuk: liafuan 3 ne'ebé la iha relasaun + numeru ida.\"",
        "13-15":"Tenta ida ne'e:\n\"Email ita mak xave prinsipal. Se ema seluk hetan, sira bele troka informasaun segredu sira hotu. Mai ita kria fraze segredu ida ne'ebé úniku no ativa verifikasaun etapa rua.\"\n\n👉 Mantén simples:\n\"Ita de'it mak lembra, hau la presiza hatene. Ita bele rai kode rekoveria iha fatin seguru.\"",
        "16+":"Tenta ida ne'e:\n\"Hau la iha hodi polisia ita. Hau iha hodi halo ita nia konta sira susar liu atu hakerek. Fraze segredu úniku + MFA ba email/sosiál/banku. Se hakarak, ita bele halo hamutuk iha minutu 10.\"",
      },
      privacy:{
        "8-12":"Tenta ida ne'e:\n\"Konfigurasaun privasidade hanesan kurtina. Se loke kurtina = ema hotu bele haree ba laran. Mai ita verifika: sé mak bele manda mensajen ba ita, tag ita, no haree ita nia story sira. No mos: taka ita boot nia lokasaun.\"",
        "13-15":"Tenta ida ne'e:\n\"Konfigurasaun privasidade hanesan kurtina. Se loke kurtina = ema hotu bele haree ba laran. Mai ita verifika: sé mak bele manda mensajen ba ita, tag ita, no haree ita nia story sira. No mos: taka ita boot nia lokasaun.\"",
        "16+":"Tenta ida ne'e:\n\"Fahe informasaun sen limite la'ós de'it foto. Ida ne'e mak padraun: ne'ebé ita ba, bainhira ita iha mesak, saida mak ita sosa. Mai ita hametin konfigurasaun sira no taka lokasaun se ita la presiza duni.\"",
      },
      boundaries:{
        "8-12":"Tenta ida ne'e:\n\"Iha ita nia família iha regra: dispozitivu sira karga iha sala moris iha kalan, no se buat ruma sente ta'uk ka segredu, hatudu kedas mai hau.\"",
        "13-15":"Tenta ida ne'e:\n\"Hau la bandu ita hodi utiliza telefone. Hau só hatuur regra seguransa. Se ema husu segredu, imajen, ka husu atu hasoru, pauza lai no konta mai hau. La iha problema se ita hatete lia los.\"",
        "16+":"Tenta ida ne'e:\n\"Ita boot ona, nune'e regra sai akordu. Ida ne'ebé la bele negoseia: se ita hetan presaun, ameasa, ka husu atu hasoru, konta mai hau kedas.\"",
      },
      strangers:{
        "8-12":"Tenta ida ne'e:\n\"Ema online bele finje sai labarik. Ita la haruka foto ba ema seluk, no la hasoru ema ida la ho adultu.\"",
        "13-15":"Tenta ida ne'e:\n\"Se ema ida husu imajen, tenta lori ita ba WhatsApp/Telegram, ka hateten 'keta konta ba ema', ida ne'e mak sinál risku (red flag). Hasai screenshot, blokeia, relata, no konta mai hau.\"",
        "16+":"Tenta ida ne'e:\n\"Se ema ida koko sexualiza konversa, husu imajen, ka ameasa ita: para hatán. Rai evidénsia. Reporta. Ita no hau sei hadia hamutuk. Ita la sei hetan kastigu tanba mai konta ba hau.\"",
      },
    } as const;
    return{
      passwords:tet.passwords[age],
      privacy:tet.privacy[age],
      boundaries:tet.boundaries[age],
      strangers:tet.strangers[age],
    };
  }
    ...
    return[
      {
        ...
        prompt:"Ema ida iha mensajen direta dehan: \"Ita nain rua deit. Haruka foto ida atu prova ita nia konfiansa.\"",
        tags:["Segredu","Imajen","Presaun"],
        options:[
          "Haruka foto tanba ita la hakarak perde kolega.",
          "Esplika katak ita boot la senti konfortavel, no relata kedan ba adultu neebe ita fiar.",
          "Para, screenshot, blokeia, reporta, no hatete ba adultu ida ne'ebé ita fiar.",
          "kria grupo konversa foun hodi prevene problema.",
        ],
        correct:2,
      ...
      },
      {
        ...
        prompt:"Ema ida online dehan: \"mai ita hasoru malu iha kafe. Ita bele mai mesak? La bele hatete ba apa ho ama.\"",
        tags:["Enkontru","Segredu","Lokasaun"],
        options:[
          "kafe ne'e fatin publiku.",
          "Husu nia naran kompletu, depois hasoru nia mesak.",
          "Hatán \"Lae\". Kualker enkontru presiza involve ema boot no planu seguransa.",
          "Fahe ita-nia lokasaun atual hodi 'mantein seguransa'.",
        ],
        correct:2,
        explain:"Kualker pedidu atu hasoru mesak no husu atu rai segredu mak sinál risku (red flag). Se enkontru akontese, tenke envolve ema boot ne'ebé ita fiar, iha fatin públiku, no iha espasu seguru.",
      },
      {
        title:"Husu kódigu (OTP)",
        prompt:"Mensajen dehan: \"Ekipa fornese suporta ba ita boot. Haruka kódigu verifikasaun (OTP) ne'ebé ita simu agora.\"",
        tags:["OTP","Konta","Fraude"],
        options:[
...
function tagIcon(tag:string){
  ...
  if(t.includes("images")||t.includes("imajen")) return <ImageIcon className="h-3.5 w-3.5 text-[#EB5757]" />;
  if(t.includes("meetup")||t.includes("enkontru")) return <MapPin className="h-3.5 w-3.5 text-[#9B51E0]" />;
  if(t.includes("otp")) return <KeyRound className="h-3.5 w-3.5 text-[#2F80ED]" />;
  if(t.includes("secrets")||t.includes("segredu")) return <LockKeyhole className="h-3.5 w-3.5 text-[#B7791F]" />;
  if(t.includes("location")||t.includes("lokasaun")) return <Smartphone className="h-3.5 w-3.5 text-[#219653]" />;
  if(t.includes("pressure")||t.includes("presaun")) return <AlertTriangle className="h-3.5 w-3.5 text-[#EB5757]" />;
