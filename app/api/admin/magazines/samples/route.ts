...
// Optional override, otherwise we'll use this default key in your bucket
const SAMPLES_JSON_KEY = process.env.AWS_S3_MAG_SAMPLES_JSON_KEY || "content/magazine-samples.json";
...
async function readSamplesFromS3():Promise<SampleRecord[]>{
  ...
  try{
    ...
    // If file doesn't exist yet, just treat as empty
    if(
      err?.name==="NoSuchKey" ||
      err?.Code==="NoSuchKey" ||
