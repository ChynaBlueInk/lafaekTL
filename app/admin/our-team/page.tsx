      ...
      <div className="mx-auto max-w-6xl">
                  ...
                  <tr key={rowKey} className={index % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                    ...
                    <td className="px-3 py-2 align-top">
                      <span
                        className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold ${
                          visible ? "bg-emerald-50 text-emerald-800" : "bg-slate-200 text-slate-700"
                        }`}
                      >
                    ...
                    <td className="px-3 py-2 align-top">
                      ...
                      <div className="text-xs text-slate-600">
                        {safeText(m.nameTet) && m.nameTet !== m.nameEn ? m.nameTet : ""}
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-2 align-top">
                      ...
                      <div className="text-xs text-slate-600">
                        {safeText(m.roleTet) && m.roleTet !== m.roleEn ? m.roleTet : ""}
                      </div>
                    </td>
        ...
        <div className="mt-4 flex items-center justify-between gap-2">
          ...
          <div className="text-xs text-slate-500">
            Tip: Use <span className="font-semibold">View</span> for checking details, and{" "}
            <span className="font-semibold">Edit</span> to update in a single form. Don’t forget{" "}
        ...
        <section className="mt-8 rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-900">
          ...
          <ul className="list-disc space-y-1 pl-5">
            <li>
              Bio fields are stored as <code>bioEn</code> and <code>bioTet</code> in <code>content/team.json</code>.
            </li>
            <li>
              Photo and sketch URLs are stored as <code>photo</code> and <code>sketch</code>, matching the public loader in <code>lib/content-team.ts</code>.
            </li>
            <li>
              Uploads use the existing <code>/api/uploads/s3/presign</code> route with the <code>our-team</code> folder.
            </li>
            <li>
              Make edits in the modal, then click <code>Save Changes</code> on this page to persist to S3.
            </li>
          </ul>
        </section>
      </div>
            ...
            <div className="overflow-y-auto p-5">
              {modalDirty && canEdit && (
                <div className="mb-4 rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-900">
                  You have unsaved popup changes. Click {modalMode === "create" ? "Create" : "Apply"} to add them to the page, then click <span className="font-semibold">Save Changes</span>.
                </div>
              )}
              ...
              <div className="grid grid-cols-1 gap-5 md:grid-cols-2">
                <div className="space-y-3">
                  <div className="grid grid-cols-1 gap-3">
                    <div>
                      <label className="mb-1 block text-xs font-medium text-slate-700">Name (English)</label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="mb-1 block text-xs font-medium text-slate-700">Name (Tetun)</label>
                      <input
                        type="text"
                        className={`w-full rounded border border-slate-300 px-2 py-1 text-xs ${
                          !canEdit ? "cursor-not-allowed bg-slate-100" : ""
                    ...
                    <div>
                      <label className="mb-1 block text-xs font-medium text-slate-700">Role (English)</label>
                      <input
                        type="text"
                        className={`w-full rounded border border-slate-300 px-2 py-1 text-xs ${
                          !canEdit ? "cursor-not-allowed bg-slate-100" : ""
                    ...
                    <div>
                      <label className="mb-1 block text-xs font-medium text-slate-700">Kargo (Tetun)</label>
                      <input
                        type="text"
                        className={`w-full rounded border border-slate-300 px-2 py-1 text-xs ${
                          !canEdit ? "cursor-not-allowed bg-slate-100" : ""
                    ...
                    <div>
                      <label className="mb-1 block text-xs font-medium text-slate-700">Started (optional)</label>
                      <input
                        type="text"
                        className={`w-full rounded border border-slate-300 px-2 py-1 text-xs ${
                          !canEdit ? "cursor-not-allowed bg-slate-100" : ""
                  ...
                  <div>
                    <label className="mb-1 block text-xs font-medium text-slate-700">Bio (English)</label>
                    <textarea
                  ...
                  <div>
                    <label className="mb-1 block text-xs font-medium text-slate-700">Bio (Tetun)</label>
                    <textarea
                      className={`h-24 w-full rounded border border-slate-300 px-2 py-1 text-xs ${
                        !canEdit ? "cursor-not-allowed bg-slate-100" : ""
                      }`}
                ...
                <div className="space-y-4">
                  <div className="rounded-xl border border-slate-200 p-4">
                    <div className="mb-2 text-xs font-semibold text-slate-700">Photo</div>
                    <div className="grid grid-cols-1 gap-3">
                  ...
                  <div className="rounded-xl border border-slate-200 p-4">
                    <div className="mb-2 text-xs font-semibold text-slate-700">Sketch</div>
                    <div className="grid grid-cols-1 gap-3">
                      <div className="flex items-center justify-center rounded-lg border border-slate-200 bg-slate-50 p-3">
                        {modalDraft.sketch ? (
                          <img
                  ...
                  {modalDraft.slug !== undefined && (
                    <div className="rounded-xl border border-slate-200 p-4">
                      <div className="mb-2 text-xs font-semibold text-slate-700">Slug (optional)</div>
                      <input
                        type="text"
                        className={`w-full rounded border border-slate-300 px-2 py-1 text-xs ${
                          !canEdit ? "cursor-not-allowed bg-slate-100" : ""
